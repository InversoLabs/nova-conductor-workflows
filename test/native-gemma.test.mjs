...
test('native Codex executes a Gemma bare-text patch through the owned proxy',{
 ...
 const upstream=http.createServer(async(req,res)=>{
  ...
  const item=count===1 ? {type:'message',id:'msg_patch',role:'assistant',status:'completed',content:[{type:'output_text',text:'*** Begin Patch\n*** Add File: proof.txt\n+provider proxy works\n*** End Patch',annotations:[]}]} : {type:'message',id:'msg_done',role:'assistant',status:'completed',content:[{type:'output_text',text:'Done.',annotations:[]}]};
  ...
  send({type:'response.output_item.added',output_index:0,item:count===1?{...item,status:'in_progress',content:[]}:item});
  if(count===1){send({type:'response.output_text.delta',item_id:item.id,output_index:0,content_index:0,delta:item.content[0].text});send({type:'response.output_text.done',item_id:item.id,output_index:0,content_index:0,text:item.content[0].text});}
 ...
 assert.equal(count,2);assert.ok(history.input.some(item=>item.type==='function_call_output'&&item.call_id.startsWith('call_')));
});
