import {editorialPolicy} from './editorial-policy.mjs';
...
const c=JSON.parse(fs.readFileSync(input,'utf8'));for(const key of ['siteUrl','remoteRoot','remotePython',...(c.transport==='local'?[]:['sshHost'])])if(typeof c[key]!=='string'||!c[key])throw Error('Missing '+key);
const template=editorialPolicy(JSON.parse(fs.readFileSync(new URL('./workflow.example.json',import.meta.url),'utf8')));
