...
import {presenterAsset} from './wardrobe.mjs';
...
try{
  ...
  await run(path.join(runtime,'voice/Scripts/python.exe'),[path.join(here,'voice.py'),'--runtime',runtime,'--script',bulletinFile?path.join(output,'bulletin.json'):path.join(here,'preview.txt'),'--output',path.join(output,'voice.wav')]);
  ...
  await run(path.join(runtime,'face/Scripts/python.exe'),[path.join(here,'compose.py'),'--ffmpeg',path.join(runtime,'bin/ffmpeg.exe'),'--input',path.join(result,masters[0]),'--output',output,...(bulletinFile?['--timeline-json',path.join(output,'voice.json'),'--headlines-json',path.join(output,'headlines.json')]:[])]);
