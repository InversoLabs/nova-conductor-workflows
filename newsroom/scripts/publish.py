...
def publish(incoming):
    ...
    try:
        ...
        if existing and not story.pop('replaceExisting',False):
            render(public); print('Already published: ' + sid); return
        if existing:
        ...
        items = ''.join('<item><title>'+escape(s['title'])+'</title><link>https://inversolabs.us/newsroom/story/'+s['id']+'</link><guid>'+s['id']+'</guid><description>'+escape(s['summary'])+'</description></item>' for s in edition['stories'][:50])
        (public / 'feed.xml').write_text('<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>IN / SIGNAL</title><link>https://inversolabs.us/newsroom/</link><description>The Inverso newsroom</description>'+items+'</channel></rss>', encoding='utf-8')
        render(public)
        print('Published '+sid)
    finally:
