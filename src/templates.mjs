...
builtins['custom-code']={schemaVersion:1,id:'CUSTOM_CODE',name:'Customizable code builder',start:'PLANNER',maxRuns:40,roles:[
  ...
  {id:'BUILDER',name:'Builder',prompt:'Read the project instructions and build plan. Implement the requested product using tools. If the latest handoff requests revisions, fix that concrete checklist while preserving working behavior. Keep planning documents unchanged. Verify the result proportionately and report evidence and limitations.',access:'workspace-write',inputs:['AGENTS.md','BUILD_PLAN.md'],outputs:[],writes:['*'],routes:{DONE:'REVIEWER',BLOCKED:'NEEDS_ATTENTION'}},
...
export function canWrite(s,f){
  ...
  return lower!=='request.md'&&r.access!=='read-only'&&!owned.has(lower)&&(r.writes.includes('*')||r.writes.some(x=>x.toLowerCase()===lower));
}
...
export function checkpoint(s,work,dir,before){
  const writable=f=>customWorkflow(s)?canWrite(s,f):s.role==='PLANNER'?['AGENTS.md','BUILD_PLAN.md'].includes(f):s.role==='REVIEWER'?['REVIEW.md','BUILD_CHECKLIST.md'].includes(f):!['AGENTS.md','BUILD_PLAN.md','REVIEW.md','REQUEST.md'].includes(f);
