...
export async function publishBulletin(folder){
  ...
  try{
    ...
    const html=`<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>The Human Briefing | ${esc(b.editionDate)} — The Artificial News</title><link rel="stylesheet" href="/fakenews/style.css"><main style="max-width:1100px;margin:40px auto;padding:24px"><a href="/fakenews/">← THE ARTIFICIAL NEWS newsroom</a><h1 style="margin:24px 0">The Human Briefing · ${esc(b.editionDate)}</h1><p>With Mara Vale${b.kind==='scheduled'?'':' · Launch-edition studio test'}</p><video controls playsinline preload="metadata" style="width:100%;background:#0d1b1b" src="website.mp4"></video><p><a href="instagram.mp4">Watch the vertical edition ↗</a></p><h2>In this bulletin</h2><ul>${links}</ul><p>The Artificial News · Human affairs. Machine perspective.</p></main></html>`;
    ...
    if(!latest||Date.parse(latest.editionCreatedAt||latest.publishedAt)<=Date.parse(b.createdAt||record.createdAt))atomic(latestFile,{id:b.id,date:b.editionDate,url,video:url+'website.mp4',title:'The Human Briefing',publishedAt:new Date().toISOString(),editionCreatedAt:b.createdAt||record.createdAt});
