...
import {selectPresenter} from '../../newsroom/anchor/wardrobe.mjs';
import {publishBulletin,recoverPublishLock} from './publish.mjs';

const here=path.dirname(fileURLToPath(import.meta.url));
export const anchorHome=()=>process.env.NOVA_FAKENEWS_ANCHOR_HOME||path.join(process.env.LOCALAPPDATA||os.homedir(),'NovaConductor','fakenews-anchor');
...
async function stories(){const r=await fetch(config().siteUrl.replace(/\/$/,'')+'/stories.json',{signal:AbortSignal.timeout(20000)});if(!r.ok)throw Error('Published newsroom feed unavailable');const data=await r.json();if(!Array.isArray(data.stories))throw Error("Invalid story feed");if(data.stories.length)latestHeadlines(data.stories);return data.stories;}
    ...
    try{const s=automationStatus();if(!s.enabled)return;fs.mkdirSync(anchorHome(),{recursive:true});
      ...
      if(studioBusy()){
        ...
        if(lease&&Number.isInteger(lease.pid)&&!alive(lease.pid)&&!s.jobs.some(j=>alive(j.pid))&&(s.recoveryAttempts||0)<3&&(!s.recoveryAt||Date.parse(s.recoveryAt)<=Date.now())){
          ...
          await promisify(execFile)(process.execPath,[path.join(here,'../../newsroom/anchor/recover.mjs')],{windowsHide:true,timeout:600000});s.recoveryAttempts=0;s.lastError='';save(s);
        }
      }
...
async function work(folder){
  ...
  try{
    ...
    const exports=read(path.join(output,'exports.json'));if(exports.some(x=>x.decoded!==true||Math.abs(x.duration-voice.duration-3)>0.5)||exports.length!==2||!exports.some(x=>x.width===1920&&x.height===1080)||!exports.some(x=>x.width===1080&&x.height===1920))throw Error('Expected both validated video formats');
